...
export default function SkillBoostTrainer(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setExerciseSets([]);
    setSelectedSetId("");
    fetch(`${API_BASE}/exercise-sets?skill=${encodeURIComponent(selectedSkill)}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSubmitAnswer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setAnswer("");
        setFeedback(null);
        setCurrentIndex((prev) => (prev + 1 < exercises.length ? prev + 1 : prev));
      }, 1200);
    } catch (e2) {
  ...
  return (
    <div
      style={{
        ...
        overflowY: "auto",
        background:
          "linear-gradient(135deg, #ffe7ba 0%, #fffbf0 40%, #c8e7ff 100%)",
      }}
    >
      <div
        style={{
      ...
      >
        ...
        >
          <div>
            <label style={{ fontSize: "0.85rem", color: "#555", display: "block" }}>
              Übungsart
            </label>
            <select
              value={selectedSkill}
        ...
        {exercises.length > 0 && (
          ...
          >
            ...
            <div style={{ textAlign: "center", marginBottom: "1rem" }}>
              <div style={{ fontSize: "0.9rem", color: "#64748b" }}>Rechne:</div>
              <div style={{ fontSize: "2.4rem", fontWeight: 700, marginTop: "0.3rem" }}>
        ...
        {exercises.length === 0 && !loading && (
          <p style={{ marginTop: "0.5rem", fontSize: "0.95rem", color: "#64748b", textAlign: "center" }}>
            Wähle oben eine Übung und tippe auf <strong>„Übung starten“</strong>, um loszulegen 🚀
          </p>
        )}
      </div>
    </div>
